...
def duplicate_checks(project_root: Path, dataset: str) -> dict[str, Any]:
    cleaned_dir = project_root / "04_data_cleaned" / dataset
...
def model_gap_summary(project_root: Path, results_root: Path) -> list[dict[str, Any]]:
    ...
    for dataset in DATASETS:
        ...
        dataset_metadata = read_json(project_root / "04_data_cleaned" / dataset / "metadata.json")
        rows.append(
            {
                "dataset": dataset,
...
def main() -> None:
    ...
    results_root = project_root / "05_analysis" / "results" / args.results_subdir
    out_dir = args.output_dir or project_root / "05_analysis" / "results" / "extratrees_sanity_check"
