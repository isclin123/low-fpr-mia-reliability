...
The processed arrays, deterministic member/non-member split files, attack-score arrays, metric tables, confidence-interval tables, audit-subsampling outputs, final table CSV files, appendix table CSV files and figure source data generated in this study are archived in the Zenodo reproducibility package at https://doi.org/10.5281/zenodo.20552369. The release excludes large trained-model cache files, which are regenerable from the documented commands, seeds, parameters and output paths. Data provenance, checksums for downloaded raw files, preprocessing notes and output manifests are recorded in `03_data_raw/data_sources.md`, `04_data_cleaned/preprocessing_log.md` and `09_final/reproducibility_package/data_and_outputs_manifest.md`.
...
The current code is organized under `05_analysis/`. The main entry points are `05_analysis/run_experiment.py`, `05_analysis/run_audit_tool.py`, `05_analysis/run_final_ci_hardening.py`, `05_analysis/generate_expanded_outputs.py` and `05_analysis/generate_final_tables.py`. Reproduction commands, environment notes, clean-environment smoke-test records and the minimum reproducible artifact path are documented in `09_final/reproducibility_package/`.
...
- Release route and licence details are recorded in `09_final/release_route_and_licence_plan.md` and `09_final/reproducibility_package/release_licence_notes.md`.
