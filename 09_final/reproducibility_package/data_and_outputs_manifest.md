...
- `03_data_raw/data_sources.md`

Preprocessing record:

- `04_data_cleaned/preprocessing_log.md`

Cleaned dataset records:

| Dataset | Cleaned metadata | Data dictionary |
|---|---|---|
| credit_default | `04_data_cleaned/credit_default/metadata.json` | `04_data_cleaned/credit_default/data_dictionary.md` |
| covertype | `04_data_cleaned/covertype/metadata.json` | `04_data_cleaned/covertype/data_dictionary.md` |
| adult_income | `04_data_cleaned/adult_income/metadata.json` | `04_data_cleaned/adult_income/data_dictionary.md` |
| bank_marketing | `04_data_cleaned/bank_marketing/metadata.json` | `04_data_cleaned/bank_marketing/data_dictionary.md` |
| diabetes_readmission | `04_data_cleaned/diabetes_readmission/metadata.json` | `04_data_cleaned/diabetes_readmission/data_dictionary.md` |
...
| Artifact | Purpose |
|---|---|
| `05_analysis/configs/expanded_tabular_v1.json` | Expanded run scope and completed-status config. |
| `05_analysis/run_experiment.py` | End-to-end dataset/model/score/metric/subsampling runner. |
| `05_analysis/run_audit_tool.py` | Reusable score-array audit CLI. |
| `05_analysis/run_final_ci_hardening.py` | Selected 1,000-bootstrap final CI hardening. |
| `05_analysis/run_q1_stronger_score_appendix.py` | Bounded reference-centered score compatibility check for the Q1 appendix. |
| `05_analysis/run_q1_lira_like_appendix.py` | Bounded shadow-model score-array compatibility check for the Q1 appendix. |
| `05_analysis/run_q1_tie_rule_sensitivity.py` | Strict/randomized/inclusive low-FPR tie-rule sensitivity check. |
| `05_analysis/run_q1_split_seed_sensitivity.py` | Bounded split/seed sensitivity check for representative target constructions. |
| `05_analysis/run_q1_label_uncertainty_stress.py` | Bounded label-corruption check for the Q1 appendix. |
| `05_analysis/generate_expanded_outputs.py` | Expanded figures and summary table generation. |
| `05_analysis/generate_final_tables.py` | Final main and appendix table generation. |
| `05_analysis/requirements-lock.txt` | Pinned dependency lock for the audit-tool smoke release path. |

## Main Result Directories

| Directory | Contents | Status |
|---|---|---|
| `05_analysis/results/expanded_tabular_stage1/` | Full five-dataset/five-model stage1 outputs with 50 bootstrap repeats and 50 subsampling repeats. | Complete. |
| `05_analysis/results/final_ci_hardening/` | Selected 1,000-bootstrap hardened intervals for main-table candidate rows. | Complete. |
| `05_analysis/results/audit_tool_example_npz/` | NPZ audit-tool example output. | Complete smoke/example validation. |
| `05_analysis/results/audit_tool_example_csv/` | CSV audit-tool example output. | Complete smoke/example validation. |
| `05_analysis/results/clean_env_smoke_npz/` | NPZ audit-tool example output from the clean virtual-environment smoke test. | Complete clean-env smoke validation. |
| `05_analysis/results/clean_env_smoke_csv/` | CSV audit-tool example output from the clean virtual-environment smoke test. | Complete clean-env smoke validation. |
| `05_analysis/results/lock_env_smoke_npz/` | NPZ audit-tool example output from the pinned-lock clean virtual-environment smoke test. | Complete locked-env smoke validation. |
| `05_analysis/results/lock_env_smoke_csv/` | CSV audit-tool example output from the pinned-lock clean virtual-environment smoke test. | Complete locked-env smoke validation. |
| `05_analysis/results/lock_env_smoke_freeze.txt` | Package freeze observed inside the pinned-lock smoke environment. | Complete locked-env smoke validation. |
| `05_analysis/results/q1_stronger_score_appendix/credit_default_random_forest/` | Reference-centered score arrays, baseline comparison metrics, and audit-tool outputs for the Q1 score check. | Complete bounded Q1 appendix check. |
| `05_analysis/results/q1_stronger_score_appendix/adult_income_random_forest/` | Adult Income reference-centered score arrays, baseline comparison metrics, and audit-tool outputs for the Q1 score check. | Complete bounded Q1 appendix check. |
| `05_analysis/results/q1_lira_like_appendix/` | Bounded shadow-model score arrays, shadow-fit summaries, key metrics, and audit-tool outputs. | Complete bounded Q1 appendix check. |
| `05_analysis/results/q1_tie_rule_sensitivity/` | Strict/randomized/inclusive low-FPR tie-rule operating-point tables. | Complete bounded Q1 sensitivity check. |
| `05_analysis/results/q1_split_seed_sensitivity/` | Repeated target-construction split/seed metric rows and summaries. | Complete bounded Q1 sensitivity check. |
| `05_analysis/results/q1_label_uncertainty_stress/credit_default_random_forest_neg_loss/` | Label-corruption repeat metrics, canonical corrupted score arrays, and canonical audit-tool outputs. | Complete bounded Q1 check. |
| `06_figures_tables/expanded/` | Expanded figures and model-comparison summary. | Regenerated with PNG/SVG/PDF figure exports. |
| `06_figures_tables/final_tables/` | Final main manuscript table package. | Complete. |
| `06_figures_tables/appendix_tables/` | Appendix metric, CI, and subsampling tables. | Complete. |
...
| Record | Purpose |
|---|---|
| `10_process_record/expanded_run_validity_report.md` | Expanded stage1 completeness, bug fix, and output-count validation. |
| `10_process_record/final_ci_hardening_report.md` | Scope and validation of selected final CI hardening. |
...
| `05_analysis/docs/audit_tool_interpretation_guide.md` | Reader-facing guide to audit-tool outputs. |
| `09_final/reproducibility_package/clean_env_smoke.md` | Clean virtual-environment smoke-test record for the audit-tool examples. |
| `09_final/reproducibility_package/locked_env_smoke.md` | Pinned-lock clean virtual-environment smoke-test record for the audit-tool examples. |
| `09_final/reproducibility_package/minimum_reproducible_artifact.md` | Smallest reviewer-facing artifact path for the score-array reporting layer. |
...
Release-route and licence/reuse details are recorded in `09_final/release_route_and_licence_plan.md` and `09_final/reproducibility_package/release_licence_notes.md`.
