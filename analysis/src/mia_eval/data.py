...
def load_credit_default(project_root: Path) -> CleanDataset:
    ...
    raw_path = project_root / "data_sources" / "credit_default" / "default of credit card clients.xls"
...
def load_covertype(project_root: Path) -> CleanDataset:
    ...
    raw_dir = project_root / "data_sources" / "covertype"
...
def load_adult_income(project_root: Path) -> CleanDataset:
    ...
    raw_dir = project_root / "data_sources" / "adult_income"
...
def load_bank_marketing(project_root: Path) -> CleanDataset:
    ...
    raw_path = project_root / "data_sources" / "bank_marketing" / "bank-additional" / "bank-additional-full.csv"
    if not raw_path.exists():
...
def load_diabetes_readmission(project_root: Path) -> CleanDataset:
    ...
    raw_path = project_root / "data_sources" / "diabetes_readmission" / "diabetic_data.csv"
    if not raw_path.exists():
...
def save_clean_dataset(
    ...
    *,
    ...
    cleaned_dir = project_root / "data_processed" / dataset.name
...
def prepare_datasets(
    ...
    *,
    ...
    log_path = root / "data_processed" / "preprocessing_log.md"
