...
FIG_DIR = ROOT / "release" / "manuscript_package" / "figures"
...
def plot_shadow_summary() -> None:
    path = ROOT / "analysis" / "results" / "q1_lira_like_appendix" / "tables" / "key_metrics.csv"
    df = pd.read_csv(path)
...
def plot_tie_rule_summary() -> None:
    path = ROOT / "analysis" / "results" / "q1_tie_rule_sensitivity" / "paper_table_tie_rule_sensitivity.csv"
    df = pd.read_csv(path)
...
def plot_split_seed_summary() -> None:
    path = ROOT / "analysis" / "results" / "q1_split_seed_sensitivity" / "paper_table_split_seed_sensitivity.csv"
    df = pd.read_csv(path)
    settings = df["setting"].tolist()
    metrics = [
