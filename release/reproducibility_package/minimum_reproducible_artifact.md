...
|---|---|
| `analysis/run_audit_tool.py` | Score-array audit CLI. |
| `analysis/src/mia_eval/` | Metric, interval, bootstrap, subsampling, plotting, and audit-tool implementation. |
| `analysis/examples/make_audit_tool_examples.py` | Generates small NPZ and CSV score-array examples. |
| `analysis/examples/example_scores.npz` | NPZ score-array smoke input. |
| `analysis/examples/example_scores.csv` | CSV score-array smoke input. |
| `analysis/run_q1_stronger_score_appendix.py` | Optional bounded reference-centered score check. |
| `analysis/run_q1_lira_like_appendix.py` | Optional bounded shadow-model score-array check. |
| `analysis/run_q1_tie_rule_sensitivity.py` | Optional tie-rule sensitivity check. |
| `analysis/run_q1_split_seed_sensitivity.py` | Optional split/seed sensitivity check. |
| `analysis/run_q1_label_uncertainty_stress.py` | Optional bounded label-corruption check. |
| `analysis/requirements.txt` | Python dependencies. |
| `analysis/requirements-lock.txt` | Pinned dependency lock used for locked-environment smoke validation. |
| `analysis/docs/audit_tool_schema.md` | Input/output schema. |
| `analysis/docs/audit_tool_interpretation_guide.md` | Reader-facing interpretation guide. |
| `release/reproducibility_package/smoke_reproduction.md` | Fast smoke commands. |
| `release/reproducibility_package/clean_env_smoke.md` | Clean-environment smoke validation record. |
...
python3 analysis/run_q1_stronger_score_appendix.py \
  ...
  --output-dir analysis/results/q1_stronger_score_appendix/credit_default_random_forest \
...
- public raw datasets under `data_sources/`;
- cleaned datasets and split files under `data_processed/`;
- `analysis/run_experiment.py` for the expanded 5 dataset x 5 model grid;
- `analysis/run_final_ci_hardening.py` for selected 1,000-bootstrap intervals;
...
- A pinned requirements lock exists at `analysis/requirements-lock.txt`.
- The pinned lock has passed the audit-tool smoke validation recorded in `release/reproducibility_package/locked_env_smoke.md`.
